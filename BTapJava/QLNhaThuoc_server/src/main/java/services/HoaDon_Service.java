...
public class HoaDon_Service extends UnicastRemoteObject implements HoaDonDAO{
	...
	public double getTongTienByMaHD(String maHD) {
	    double tongTien = 0;
	    try {
	        String sql = "SELECT hd.tongTien FROM HoaDon hd WHERE hd.maHD = :maHD";
	        Query query = entityManager.createQuery(sql);
	        query.setParameter("maHD", maHD);
	        Object result = query.getSingleResult();
	        if (result != null) {
	...
	public boolean updateTongTien(String maHD, double additionalAmount) throws RemoteException {
		...
		try {
			...
			transaction.begin();
			String sql = "UPDATE HoaDon hd SET hd.tongTien = hd.tongTien + :additionalAmount WHERE hd.maHD = :maHD";
			Query query = entityManager.createQuery(sql);
			query.setParameter("additionalAmount", additionalAmount);
			query.setParameter("maHD", maHD);
    ...
    public HoaDon getHoaDonByMaHD(String maHD) throws RemoteException {
        HoaDon hoaDon = null;
        try {
            Query query = entityManager.createQuery("SELECT h FROM HoaDon h WHERE h.maHD = :maHD");
            query.setParameter("maHD", maHD);
