...
public class NhanVien_Service extends UnicastRemoteObject implements NhanVienDAO {
	...
	public int getMaxEmployeeID() throws RemoteException {
	    int maxEmployeeID = 0;
	    Query query = entityManager.createQuery("SELECT MAX(CAST(SUBSTRING(nv.maNV, 3, LENGTH(nv.maNV) - 2) AS INTEGER)) FROM NhanVien nv");
	    Object result = query.getSingleResult();
	    if (result != null) {
	        maxEmployeeID = ((Number) result).intValue();
	    }
