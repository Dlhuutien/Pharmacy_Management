...
public class QuanLyNhanVien_GUI extends JPanel implements ActionListener, MouseListener {
	...
	public QuanLyNhanVien_GUI() throws Exception {
	...
	private void createNhanVienComponents() throws RemoteException {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnTimNhanVien)) {
			...
			NhanVien nv = null; 
			try {
				nv = nv_dao.getNhanVienTheoMaNV(txtTimNhanVien.getText().trim());
			} catch (RemoteException e1) {
				e1.printStackTrace();
			}
			if (nv != null) {
				...
				JOptionPane.showMessageDialog(this,
						"Không tìm thấy nhân viên có mã " + txtTimNhanVien.getText().trim());
			}
		}
		...
		if (o.equals(btnSuaNhanVien)) {
			...
			if (checkValid()) {
				...
				try {
					success = nv_dao.update(updatedNhanVien);
				} catch (RemoteException e1) {
					e1.printStackTrace();
				}
	...
	private String generateEmployeeID() throws RemoteException {
	...
	public NhanVien taoNhanVien() throws RemoteException {
